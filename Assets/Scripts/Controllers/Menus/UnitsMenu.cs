...
public class UnitsMenu : Menu {
	...
	// Temporary Variables for the Menu
	...
	void Start() {
		...
		_guiCamera = transform.root.GetComponent<Camera>();
	...
	public override void OnMenuActivityChange(bool b) {
		base.OnMenuActivityChange(b);
		if (!b) {
			_organUnitsMenu.gameObject.SetActive(false);
			_selUnitsMenu.gameObject.SetActive(false);
